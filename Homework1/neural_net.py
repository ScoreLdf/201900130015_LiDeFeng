...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        dscore[range(N), y] -= 1
        dscore /= N
        grads['W3'] = np.dot(hidden_layer_2.T, dscore) + reg * W3
        grads['b3'] = np.sum(dscore, axis = 0)
 
        dhidden_2 = np.dot(dscore, W3.T)
        dhidden_2[hidden_layer_2 <= 0.00001] = 0
 
        grads['W2'] = np.dot(hidden_layer_1.T, dhidden_2) + reg * W2
        grads['b2'] = np.sum(dhidden_2, axis = 0)
 
        dhidden_1 = np.dot(dhidden_2, W2.T)
        dhidden_1[hidden_layer_1 <= 0.00001] = 0
 
        grads['W1'] = np.dot(X.T, dhidden_1) + reg * W1
    ...
    def predict(self, X):
        ...
        hidden_layer1 = np.maximum(0, np.dot(hidden_layer, W2) + b2)
        scores = np.dot(hidden_layer1, W3) + b3
